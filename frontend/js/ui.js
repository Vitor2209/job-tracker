...
const UI = {
    ...
    createJobCard(job) {
        ...
        const noteTag = job.notes
            ? `<span class="job-card-tag tag-${job.status}">${job.notes}</span>`
            : '';

        const statusTag = job.notes
            ? ''
            : `<span class="job-card-tag tag-${job.status}">${dateStr}</span>`;

        card.innerHTML = `
            <div class="job-card-actions">
                <button class="card-action-btn edit-btn" data-id="${job._id}" title="Edit">
                ...
                <button class="card-action-btn delete-btn" data-id="${job._id}" title="Delete">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    renderBoard(jobs) {
        ...
        Object.entries(columns).forEach(([status, list]) => {
            ...
            list.forEach(job => {
                ...
                Kanban.makeCardDraggable(card, job._id);
